...
let tries = 0;
const maxTries = 5;
console.log("Initial tries count:", tries);
console.log("Max tries allowed:", maxTries);

// Function to handle user guess
function handleGuess() {
    let wordinput = getInputValue();
    console.log("User input:", wordinput);
    if (!isValidWord(wordinput)) return; // Check if the word is valid
    tries++; // Increment the tries counter
    console.log("Updated tries count:", tries);
    updateBoard(wordinput); // Update the board with the user's guess
    checkGameOver(wordinput); // Check if the game is over
    document.getElementById("textBox").value = ""; // Clear the input box after submission
}

// Get the current input value from the text box
function getInputValue() {
    let inputValue = document.getElementById("textBox").value.trim().toLowerCase();
    console.log("Current input value:", inputValue);
    return inputValue;
}

// Validate the user's word input (length check and existence check)
function isValidWord(word) {
    console.log("Checking if word is valid:", word);
    if (word.length !== 4) {
        alert("Error: Word must be exactly 4 letters long");
        console.log("Invalid word length:", word.length);
        return false;
    }
    if (!fourwords.includes(word)) {
...
// Update the board with the user's guess
function updateBoard(word) {
    let startLetter = (tries - 1) * 4 + 1; // Calculate starting cell for this row of guesses
    console.log("Updating board for word:", word);
    for (let i = 0; i < 4; i++) {
        let letter = word.charAt(i); // Get the letter from the word
        let randletter = rand.charAt(i); // Get the corresponding letter from the random word
        let cell = document.getElementById((startLetter + i).toString()); // Get the corresponding cell
        console.log(`Updating cell ${startLetter + i}:`, letter, "Expected letter:", randletter);
        cell.innerHTML = letter; // Set the letter in the cell
        cell.style.backgroundColor = getLetterColor(letter, randletter, rand); // Set the color based on correctness
    }
}

// Determine the color for the letter based on its correctness
function getLetterColor(letter, randletter, rand) {
    console.log("Checking color for letter:", letter);
...
function checkGameOver(word) {
    console.log("Checking if game is over for word:", word);
    document.getElementById('word').textContent = "The word was: " + rand; // Display the correct word
    if (word === rand) {
        gameWon(); // Call gameWon() if the user guessed the word correctly
...
// Close the popup after the game is over
function closePopup() {
